function testOutput = getModel(trainData, testData, modelType)
 % Here we define the model which is used for the prediction
 % We will call ErrorFunc(trainData,parameters) to update the parameters
 % test output is the predicted value on testData using the learned model Vector of predicted PM2.5
 ...
 elseif modelType == "gaussianProcess"
     ...
     % UPDATED - This is now updated back!!!! Tarjei - We are testing speed using only one x-variable. For the
     % original x_train variables (all of the variables) see the commented
     % out array commented out above this
     % Also note the comment out for 47-49 60-65, 76-77
     ...
     d = size(x_train,2);
     kparams = 5*ones(d,1); % This is the parameter need to be optimised and its importance has to be understood
     sigma0 = 1;
 ...
 % (Andrew) This line depends on the 'modelType' being 'gaussianProcess'.
...
%  ylabel('Log of length scale');
 ...
 %disp(getAccuracy(testOutput,Ytest));
